...
import { useDataMSTContext } from "@/components/context/DataMSTContext";

export default function MSTDataBase() {
  const { selectedDataset } = useDataMSTContext();
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `/api/shortestpath/data?fileName=${selectedValue}`
        );
        if (!response.ok) {
